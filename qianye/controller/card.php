...
class card extends Admin
{
	...
	public function exportAction()
	{
		...
		$list = $this->db->getAll(
			null,
			null,
			"id,cardtypename,code,concat('\'',pass),customermobile,from_unixtime(createtime,'%Y-%m-%d'),createby,from_unixtime(saletime,'%Y-%m-%d'),saleby,from_unixtime(activetime,'%Y-%m-%d'),from_unixtime(exptime,'%Y-%m-%d'),case when status=10 then '未销售' when status=20 then '销售' when status=30 then '激活' when status=40 then '失效' else '其它' end",
			'id desc'
		);
